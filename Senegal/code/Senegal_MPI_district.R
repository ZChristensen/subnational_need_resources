...
rawMPI=read.dta("sen_dhs16_pov.dta",convert.factors = T)
...
weights=sum(rawMPI$sample.weight,na.rm=T)
district$share.of.pop=district$weights/weights
district=subset(district, !is.na(region))
fwrite(district,"E:/git/subnational_need_resources/Senegal/input/Senegal_MPI_district.csv")
